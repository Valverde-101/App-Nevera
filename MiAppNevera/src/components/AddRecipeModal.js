...
export default function AddRecipeModal({
  ...
  const resizeText = pct => {
    if (isWeb) {
      ...
      const fonts = webEditor.current?.querySelectorAll('font[size="7"]');
      fonts.forEach(f => {
        f.removeAttribute('size');
        f.style.fontSize = pct;
      });
      handleWebChange();
    } else {
      richText.current?.commandDOM?.(`(function(){
          ...
          {isWeb ? (
            <>
              ...
              <View style={styles.richBar}>
                ...
                <TouchableOpacity
                  onPress={() => resizeText('150%')}
                  style={styles.richBtn}
                >
                  <Text style={{ color: palette.text, fontSize: 16 }}>A+</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => resizeText('100%')}
                  style={styles.richBtn}
                >
                  <Text style={{ color: palette.text, fontSize: 16 }}>A</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => resizeText('75%')}
                  style={styles.richBtn}
                >
                  <Text style={{ color: palette.text, fontSize: 14 }}>A-</Text>
                </TouchableOpacity>
            ...
            <>
              ...
              <RichToolbar
                ...
                iconMap={{
                  ...
                  fontBig: ({ tintColor }) => (
                    <Text style={{ color: tintColor, fontSize: 14 }}>A+</Text>
                  ),
                  fontNormal: ({ tintColor }) => (
                    <Text style={{ color: tintColor, fontSize: 14 }}>A</Text>
                  ),
                  fontSmall: ({ tintColor }) => (
                    <Text style={{ color: tintColor, fontSize: 12 }}>A-</Text>
                  ),
