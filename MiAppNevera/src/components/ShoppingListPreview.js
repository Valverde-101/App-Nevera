      ...
      <ScrollView style={[styles.scroll, style]} contentContainerStyle={{ padding: 14, paddingBottom: 40 }}>
        ...
        {items.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>{t('label.list_details')}</Text>
            </View>
            <View style={styles.detailsRow}>
              <TouchableOpacity style={styles.actionBtn} onPress={() => setDetailsVisible(true)}>
                <Text style={styles.actionText}>{t('btn.more_details')}</Text>
              </TouchableOpacity>
              <Text style={styles.totalText}>{t('label.total_cost', { symbol, cost: totalCost.toFixed(2) })}</Text>
            </View>
          </View>
        )}
      </ScrollView>
          ...
          <View style={styles.modalBackdrop}>
            <TouchableWithoutFeedback>
              <View style={[styles.card, { alignItems: 'center' }]}>
                <Text style={[styles.cardTitle, { marginBottom: 16 }]}>{t('modal.cost_distribution.title')}</Text>
                {totalCost > 0 ? (
                  <>
                  ...
                  <Text style={{ color: palette.textDim }}>{t('msg.no_cost_data')}</Text>
                )}
              </View>
            </TouchableWithoutFeedback>
          </View>
