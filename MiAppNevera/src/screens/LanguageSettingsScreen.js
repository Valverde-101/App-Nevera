...
export default function LanguageSettingsScreen({ navigation }) {
  ...
  useLayoutEffect(() => {
    nav.setOptions?.({
      title: t('system.settings.languageTitle'),
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={{ padding: 16 }}
        ...
        <TouchableOpacity style={styles.item} onPress={() => setLang('es')}>
          <Text style={styles.itemTitle}>{t('system.language.spanish')}</Text>
          {lang === 'es' && <Text style={styles.itemDesc}>✓</Text>}
        </TouchableOpacity>
        <TouchableOpacity style={styles.item} onPress={() => setLang('en')}>
          <Text style={styles.itemTitle}>{t('system.language.english')}</Text>
          {lang === 'en' && <Text style={styles.itemDesc}>✓</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
