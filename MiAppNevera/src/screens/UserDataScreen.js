...
export default function UserDataScreen() {
  ...
  const exportData = async () => {
    try {
          ...
          if (Array.isArray(rec.ingredients)) {
            for (const ing of rec.ingredients) {
              ...
              if (typeof iconUri === 'string') {
                ...
                if (iconPath) ing.icon = { uri: iconPath };
              }
            }
          }
        }
      ...
      const zipName = `RefriMudanza_${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}.zip`;
