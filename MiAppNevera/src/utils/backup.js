...
const isCustomUri = uri => {
  ...
  if (uri.startsWith('file://') || uri.startsWith('data:') || uri.startsWith('blob:')) {
    return true;
  }
  if (uri.startsWith('http')) {
    if (Platform.OS === 'web') {
      try {
        ...
        if (origin === window.location.origin && pathname.includes('/icons/')) {
          return false;
        }
      } catch {}
    }
...
export const exportBackup = async () => {
  try {
    ...
    const addIcon = async uri => {
      if (!uri || !isCustomUri(uri)) return null;
      try {
    ...
    if (Array.isArray(data.customCategories)) {
      for (const cat of data.customCategories) {
        if (typeof cat.icon === 'string' && isCustomUri(cat.icon)) {
          const path = await addIcon(cat.icon);
          if (path) cat.icon = path; else delete cat.icon;
        } else {
          delete cat.icon;
        }
      }
    ...
    if (Array.isArray(data.customFoods)) {
      for (const food of data.customFoods) {
        if (typeof food.icon === 'string' && isCustomUri(food.icon)) {
          const path = await addIcon(food.icon);
          if (path) food.icon = path; else delete food.icon;
        } else {
          delete food.icon;
        }
      }
    ...
    if (Array.isArray(data.locations)) {
      for (const loc of data.locations) {
        if (typeof loc.icon === 'string' && isCustomUri(loc.icon)) {
          const path = await addIcon(loc.icon);
          if (path) loc.icon = path; else delete loc.icon;
        }
      }
    }
    ...
    if (Array.isArray(data.recipes)) {
      for (const rec of data.recipes) {
        if (typeof rec.image === 'string' && isCustomUri(rec.image)) {
          const imgPath = await addImage(rec.image);
          if (imgPath) rec.image = imgPath; else delete rec.image;
        } else {
          delete rec.image;
        }
        if (Array.isArray(rec.ingredients)) {
          for (const ing of rec.ingredients) {
            ...
            if (typeof iconUri === 'string' && isCustomUri(iconUri)) {
...
export const importBackup = async () => {
  try {
    ...
    if (Platform.OS === 'web') {
      ...
      if (!file) return;
      zipData = await new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = () => {
          const result = reader.result;
          resolve(result instanceof ArrayBuffer ? new Uint8Array(result) : result);
        };
        reader.onerror = reject;
      ...
      zipData = await FileSystem.readAsStringAsync(fileUri, {
        encoding: FileSystem.EncodingType.Base64,
      });
    }
